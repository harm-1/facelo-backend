...
from .serializers import ChallengeSchema, challenge_schemas

blueprint = Blueprint("challenge", __name__)

from facelo.constants import (CHALLENGE_TYPE_DISTIBUTION,
                              RECENT_CHALLENGES_COUNT)


@blueprint.route("/question/<question_id>/challenges", methods=["GET"])
@jwt_required()
@marshal_with(ChallengeSchema(many=True))
def get_challenges(question_id: int):
    question = Question.get_by_id(question_id)
    challenges = get_recent_challenges(current_user, question,
                                       RECENT_CHALLENGES_COUNT)
    ...
    generated = generate_challenges(CHALLENGE_TYPE_DISTIBUTION, uncompleted,
...
def put_challenges(*challenges, **kwargs):
    for chall_result in challenges:
        chall_ori = Challenge.get_by_id(
            chall_result["id"]).complete(chall_result)

    return ("", 204)


def get_recent_challenges(user: User,
                          question: Question,
                          size=62) -> list[Challenge]:
    latest = Challenge.query.filter_by(
        judge=user, question_id=question.id).order_by(
