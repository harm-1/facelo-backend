...
def generate_challenges(to_create: dict[int, int], uncompleted: list[Challenge],
                        completed: list[Challenge], user: User) -> list[dict[str, int]]:
    ...
    generated_triangle = gen_triangle(to_create[CHALLENGE_TYPE_TRIANGLE], completed, user,
                                      not_in_filter)

    not_in_filter.extend(generated_triangle)
    generated_sametrial = gen_sametrial(to_create[CHALLENGE_TYPE_SAMETRIAL], completed, user,
                                        not_in_filter)

    not_in_filter.extend(generated_sametrial)
    generated_random = gen_random(to_create[CHALLENGE_TYPE_RANDOM], user, not_in_filter)
...
def created_challenges_list(challenges: list[Challenge]) -> list[list[Trial]]:
    return [
        sorted([challenge.winner, challenge.loser], key=lambda k: (k is None, k))
        for challenge in challenges
    ]


def gen_random(size: int, user: User, not_in_filter: list[list[Trial]]) -> list[list[Trial]]:
